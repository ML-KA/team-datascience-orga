...
def createUploadFile(model_class, train_data, test_data, name, params={}):
    train_values = train_data["train_values"]
    train_target = train_data["train_target"]
...
def main():
    ...
    test_data = loadDataTest()
    createUploadFile(svm.SVC, train_data, test_data, "svcNoMod.csv", {"probability": True})

    # Scores with remove Volume from data, direct correlation with num of donations
    train_data = loadDataTrainNoVolume()
    test_data = loadDataTestNoVolume()
    #score 0.5335
    createUploadFile(svm.SVC, train_data, test_data, "svcRemVol.csv", {"probability": True})

    # Scores with remove Volume and oversampling
    train_data = loadDataTrainOversampled()
    test_data = loadDataTestNoVolume()
    #score 0.6093
    createUploadFile(svm.SVC, train_data, test_data, "svcOversampling.csv", {"probability": True})
    ...
    test_data = loadDataTestNoVolume()
    #score 0.6296
    createUploadFile(svm.SVC, train_data, test_data, "svcEvenlyDistri.csv", {"probability": True})
